...
//校时结构参数
    ...
    public class NET_DVR_VIDEO_CALL_PARAM extends Structure{
        public int dwSize;
        public int dwCmdType;
        public short  wPeriod;
        public short     wBuildingNumber;
        public short     wUnitNumber;
    ...
    public static class NET_DVR_TIME_EX extends Structure{
        ...
        public NET_DVR_TIME_EX(short wYear, byte byMonth, byte byDay, byte byHour, byte byMinute, byte bySecond, byte byRes) {
            this.wYear = wYear;
...
//NET_DVR_Login()参数结构
    ...
    */
    ...
    public static class NET_DVR_ID_CARD_INFO_ALARM extends Structure {
        ...
        public float fTemperatureValue;//温度值；单位摄氏度，精确到0.001。若为0xff表示无效值
        public NET_VCA_POINT struPoint;//人脸框内最高温度点坐标
        public byte byTemOver; //温度是否超标, 0-未超标，1- 超标
        public byte[] byRes3 = new byte[3];
        public int dwHeatMapDataLen;   //热度图数据长度，不为0是表示后面带数据
        public int dwVisiDataLen;   //测温仪的可见光数据长度，不为0是表示后面带数据
        public int dwBufDataLen;   //热度矩阵数据长度，不为0是表示后面带数据
        public Pointer pHeatMapData;   //热度图图片指针
        public Pointer pVisiData;       //测温仪的可见光图片指针
        public Pointer pBufData;       //热度矩阵数据
...
//开锁记录
    ...
    public static class NET_DVR_ACS_EVENT_INFO_EXTEND_V20 extends Structure{
        ...
        public int dwQRCodeInfoLen; //二维码信息长度，不为0是表示后面带数据  定制说明：当刷卡类型为二维码（bySwipeCardType=1），该字段为二维码内容的长度，结构体后面带的数据为二维码数据。
    ...
    public static class NET_DVR_ACS_EVENT_INFO_EXTEND extends Structure{
        ...
        public byte[]  byRes2 =  new byte[2];
        public byte[]  byUUID = new byte[36/*36*/]; //UUID（该字段仅在对接萤石平台过程中才会使用）
        public byte[]  byDeviceName = new byte[64];   //设备序列号
        public byte[]   byRes = new byte[24];
    }
    ...
    public static class NET_DVR_ACS_EVENT_INFO extends Structure{
        ...
        public int dwSize;
        public byte[] byCardNo =  new byte[ACS_CARD_NO_LEN]; //卡号，为0无效（18446744073709551613为超级码，18446744073709551614为胁迫码，18446744073709551615为非法卡）【定制说明】：刷二维码情况，卡号固定为123456789
        ...
        public int dwCardReaderNo; //读卡器编号，为0无效
        public int dwDoorNo; //门编号(楼层编号)，为0无效（当接的设备为人员通道设备时，门1为进方向，门2为出方向）
        ...
        public byte  bySwipeCardType;//刷卡类型，0-无效，1-二维码  定制说明：上传二维码时该字段为1
        public byte  byRes2;
        public int dwSerialNo; //事件流水号，为0无效   定制说明：下面的二维码远程核验接口返回的流水号为该字段值。
    ...
    public static class NET_DVR_ACS_ALARM_INFO extends Structure{
        ...
        public byte    byAcsEventInfoExtendV20;    //pAcsEventInfoExtendV20是否有效：0-无效，1-有效（byAcsEventInfoExtendV20有效时，byAcsEventInfoExtend一定有效）
...
//透明通道设置
    ...
    boolean NET_DVR_EncodeG722Frame(Pointer pEncodeHandle, NET_DVR_AUDIOENC_PROCESS_PARAM param);
    boolean NET_DVR_EncodeG711Frame(Pointer pEncodeHandle, NET_DVR_AUDIOENC_PROCESS_PARAM param);

    /** 音频编码信息结构体(Linux版本使用)。 **/
    public static class NET_DVR_AUDIOENC_INFO extends Structure {
        public int in_frame_size;// 输入一帧音频数据的大小(BYTES)：如果是G711编码，输入数据的大小为320字节；如果是G722编码，输入数据的大小为1280字节
        public byte[] reserved = new byte[16];
    }

    public static class NET_DVR_AUDIOENC_PROCESS_PARAM extends Structure {
        public Pointer in_buf;// 输入数据缓冲区，存放编码前PCM原始音频数据
        public Pointer out_buf;// 输出数据缓冲区，存放编码后音频数据
        public int out_frame_size;// 编码后一帧音频数据的大小(单位为字节)：如果是G711编码，输入数据的大小为160字节；如果是G722编码，输入数据的大小为80字节
        public byte g726enc_reset; // G726重置开关(保留)
        public byte g711_type; // G711编码类型：0- U law，1- A law
        public int enc_mode; // AMR音频编码模式配置(保留)
        public byte[] reserved = new byte[16];
    }
...
{
    PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary(HCNetSDK.sdk_path+"PlayCtrl.dll",PlayCtrl.class);
